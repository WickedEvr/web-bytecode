...
UPDATE countries SET tax_id_regex = '^([A-ZÑ&]{3,4})\d{6}([A-V1-9][A-Z1-9]\d{0,1})?$', tax_id_format = 'RFC Válido' WHERE iso2 = 'MX';
UPDATE countries SET tax_id_regex = '^\d{2}\-\d{7}$', tax_id_format = 'EIN (Ej. 12-3456789)' WHERE iso2 = 'US';
UPDATE countries SET tax_id_regex = '^[A-Z]\d{7}[A-Z0-9]$', tax_id_format = 'NIF Válido' WHERE iso2 = 'ES';
