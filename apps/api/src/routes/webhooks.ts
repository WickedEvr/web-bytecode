    ...
    if (action === 'opened' || action === 'synchronize' || action === 'reopened') {
      ...
      try {
        ...
        try {
          ...
          // 2. Esperar 20 segundos para que Postgres termine su reinicio interno de inicialización
          await new Promise(resolve => setTimeout(resolve, 20000));

          // 3. Crear la base de datos destino si por alguna razón no se creó sola (ignoramos el error)
          await execAsync(`docker exec bytecode-db-pr-${prNumber} createdb -U bytecode_user bytecode_prod`).catch(() => {});

          // 4. Cruzar los datos: Volcar desde Prod (limpiando basura previa con -c) y empujar a la DB Efímera
          ...
          // 5. Reiniciar el backend efímero para que se conecte a la base de datos ya poblada
