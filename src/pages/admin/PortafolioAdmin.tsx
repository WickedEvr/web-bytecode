          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-start justify-between gap-4 border-b border-white/5 pb-4">
              <div>
                ...
                <p className="mt-1 text-xs text-white/40">{selectedItem ? `Actualizado: ${formatDate(selectedItem.updated_at)}` : 'Completa los datos y guarda para subir imagen.'}</p>
              </div>
              {selectedId && (
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <label className="grid gap-1.5">
                <span className="text-[10px] uppercase tracking-wider text-white/40">Empresa / cliente</span>
                <input value={form.clientName} onChange={(event) => setForm({ ...form, clientName: event.target.value })} className="rounded-lg border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white/90 outline-none transition focus:border-white/30" />
              </label>
              ...
              <label className="grid gap-1.5 md:col-span-2">
                <span className="text-[10px] uppercase tracking-wider text-white/40">Descripcion</span>
                <textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} rows={3} className="resize-none rounded-lg border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white/90 outline-none transition custom-scrollbar focus:border-white/30" />
              </label>
              <label className="grid gap-1.5">
              ...
              <div className="grid gap-4 border-t border-white/5 pt-5">
                ...
                <div className="grid gap-5 lg:grid-cols-[220px_1fr]">
                  ...
                  <div className="grid gap-3">
                    ...
                    <button onClick={handleUploadImage} disabled={saving || !imageFile} className="inline-flex items-center justify-center gap-2 rounded-lg border border-white/10 bg-white/5 px-4 py-2.5 text-sm font-medium text-white/80 transition hover:bg-white/10 disabled:opacity-50">
