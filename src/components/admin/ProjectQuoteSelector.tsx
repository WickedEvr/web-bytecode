...
import { formatPen } from '../../hooks/useQuoterState';
...
const ProjectQuoteSelector: React.FC<Props> = ({ email, value, onChange }) => {
  ...
  useEffect(() => {
    ...
    fetchProjectQuotesByEmail(email)
      .then((items) => { if (active) setQuotes(items); })
  ...
  const options = [
    ...
    ...quotes.map((quote) => ({
      value: quote.id,
      label: `${quote.quote_code} - ${formatPen(Number(quote.total_amount))}`,
    })),
  ];
